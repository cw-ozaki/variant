...
smoke23:
	rm -rf dist/$(VERSION) && rm -rf dist/$(VERSION)-linux && GOOS=linux GOARCH=amd64 make build && mkdir dist/$(VERSION)-linux && mv dist/$(VERSION)/var dist/$(VERSION)-linux/var
	rm -rf dist/$(VERSION) && rm -rf dist/$(VERSION)-darwin && GOOS=darwin GOARCH=amd64 make build && mkdir dist/$(VERSION)-darwin && mv dist/$(VERSION)/var dist/$(VERSION)-darwin/var
	rm -rf dist/$(VERSION) && make build

	cd $(IT_DIR) && export PATH=$(shell pwd)/dist/$(VERSION):$$PATH && VAR_PATH=$(shell pwd)/dist/$(VERSION)-linux/var ./containerized-task-variant-inject inject --verbose --logtostderr | grep "task1" && echo smoke24 passed.
